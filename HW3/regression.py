...
class Regression(object):
    ...
    def linear_fit_GD(
        ...
        """
        N, D = xtrain.shape
        weight = np.zeros((D, 1)) #initialize weights
        loss_per_epoch = []
        
        for epoch in range(epochs):
            pred = np.dot(xtrain, weight)
            gradient = (1 / N) * np.dot(xtrain.T, (pred - ytrain))
            weight = weight - learning_rate * gradient #update weight
            
            pred = np.dot(xtrain, weight)
            loss_per_epoch.append(self.rmse(pred, ytrain)) #get RMSE after update
    ...
    def linear_fit_SGD(
        ...
        for epoch in range(epochs):
            for i in range(N):
                #get single training instance
                xi = xtrain[i:i+1]
                yi = ytrain[i:i+1]
                
                pred_i = np.dot(xi, weight)
                gradient = np.dot(xi.T, (pred_i - yi))
                weight = weight - learning_rate * gradient
                
                pred = np.dot(xtrain, weight)
                loss_per_step.append(self.rmse(pred, ytrain))
    ...
    def ridge_fit_closed(
        ...
        return np.dot(np.linalg.pinv(np.dot(xtrain.T, xtrain) + c_lambda * I), np.dot(xtrain.T, ytrain))
    ...
    def ridge_fit_GD(
        ...
        for epoch in range(epochs):
            ...
            reg_term = np.copy(weight)
            reg_term[0] = 0  #no regularization for bias term
            gradient += (c_lambda / N) * reg_term
            weight = weight - learning_rate * gradient 
            
            pred = np.dot(xtrain, weight)
            loss_per_epoch.append(self.rmse(pred, ytrain))
    ...
    def ridge_fit_SGD(
        ...
        for epoch in range(epochs):
            for i in range(N):
                ...
                #add regularization 
                reg_term = np.copy(weight)
                reg_term[0] = 0  #no reg for bias term
                gradient += (c_lambda / N) * reg_term
                weight = weight - learning_rate * gradient
    ...
    def ridge_cross_validation(
        ...
        for k in range(kfold):
            ...
            test_end = (k + 1) * fold_size if k < kfold - 1 else N
            ...
            fold_rmse = self.rmse(y_pred, y_test)
