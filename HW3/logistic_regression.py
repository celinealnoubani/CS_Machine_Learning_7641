...
class LogisticRegression(object):
    ...
    def fit(
        ...
        x_train_aug = self.bias_augment(x_train) #augment training data
        
        #training loop
        for epoch in range(epochs):
            h_x = self.predict_probs(x_train_aug, theta)
            grad = self.gradient(x_train_aug, y_train, h_x)
            theta = theta - lr * grad #update theta w/ gradient descent 
        
            if epoch % 100 == 0:
                self.update_evaluation_lists(x_train, y_train, x_val, y_val, theta, epoch, threshold)
        
        if (epochs - 1) % 100 != 0:
            self.update_evaluation_lists(x_train, y_train, x_val, y_val, theta, epochs - 1, threshold)
...
def hyperparameter_tuning(
    ...
    for threshold in thresholds:
        loss, accuracy = model.evaluate(x_test, y_test, theta, threshold)
