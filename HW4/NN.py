...
class NeuralNet:
    ...
    def softmax(self, u):
        ...
        """
        exp_logits = np.exp(u - np.max(u, axis=1, keepdims=True))
    ...
    def silu(self, u):
        ...
        """
        u_safe = np.clip(u, -30.0, 30.0)
        sigmoid = 1 / (1 + np.exp(-u_safe))
        return u * sigmoid
    ...
    def derivative_silu(self, x):
        ...
        """
        x_safe = np.clip(x, -30.0, 30.0)
        sigmoid = 1 / (1 + np.exp(-x_safe))
    ...
    def update_weights(self, dLoss):
        ...
        if self.use_adam:
                for param in ["theta1", "b1", "theta2", "b2", "theta3", "b3"]:
                        dLoss[param] = np.clip(dLoss[param], -1.0, 1.0)
                
                        # Update first and second moments
                        self.first_moment[param] = self.beta1 * self.first_moment[param] + (1 - self.beta1) * dLoss[param]
                        self.second_moment[param] = self.beta2 * self.second_moment[param] + (1 - self.beta2) * (dLoss[param]**2)
                
                        # Correcting bias
                        first_moment_corrected = self.first_moment[param] / (1 - self.beta1**self.t)
                        second_moment_corrected = self.second_moment[param] / (1 - self.beta2**self.t)
                        denominator = np.sqrt(second_moment_corrected) + self.eps
                        update = np.clip(first_moment_corrected / denominator, -1.0, 1.0)
                        
                        self.parameters[param] -= self.learning_rate * update
        else:
                for param in ["theta1", "b1", "theta2", "b2", "theta3", "b3"]:
                        dLoss[param] = np.clip(dLoss[param], -1.0, 1.0)
    ...
    def backward(self, y, yh):
        ...
        """
        dLoss = self.compute_gradients(y, yh)
        self.update_weights(dLoss)
    ...
    def minibatch_gradient_descent(self, x, y, iter=60000, local_test=False):
        ...
        for i in range(iter):
                ...
                if end_idx <= N:
                        ...
                        wrapped_idx = end_idx - N
                        x_batch = np.vstack((x[start_idx:], x[:wrapped_idx]))
                        y_batch = np.vstack((y[start_idx:], y[:wrapped_idx]))
                ...
                self.iteration = (self.iteration + 1) % (N // self.batch_size + 1)
                self.t += 1
