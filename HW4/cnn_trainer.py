...
class Trainer:
    ...
    def run_epoch(self, total, correct, running_loss, data_iterator, train=True):
        ...
        for i, (inputs, labels) in enumerate(data_iterator):
            ...
            # For training: backpropagation and optimization
            if train:
            ...
            if train and total > 0:  # Ensure we don't divide by zero
                data_iterator.set_postfix(
                    loss=f"{running_loss/(i+1):.4f}",  # Use iteration count instead of data_iterator.n
                    accuracy=f"{correct/total:.4f}")
    
        # Update learning rate scheduler if in training mode
        if train:
            self.scheduler.step()
