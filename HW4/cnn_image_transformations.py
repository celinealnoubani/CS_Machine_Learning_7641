...
def create_training_transformations():
    ...
    """
    return v2.Compose([
    v2.ToImage(),  # PIL to tensor
    v2.RandomRotation(degrees=(-10, 10)),  # random rotation
    v2.RandomAffine(degrees=0, translate=(0.05, 0.05)),  # random affine transformation
    v2.RandomHorizontalFlip(p=0.5),  # random horizontal flip
    v2.ToDtype(torch.float32, scale=True)]) # convert & scale
